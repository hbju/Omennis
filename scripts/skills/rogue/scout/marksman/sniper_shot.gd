...
func score_action(from: CombatCharacter, potential_targets: Array[CombatCharacter], _target_cell: Vector2i, map: CombatMap) -> float:
	...
	var score = AIScoringWeights.WEIGHT_BASE_RANGED * 1.5 # High value for a powerful shot
	
	var potential_damage = from.get_damage() * damage_mult
	
	# Bonus damage from the Marksman passive if applicable
	var dist = HexHelper.distance(map.get_cell_coords(from.global_position), _target_cell)
	if dist >= 5:
		potential_damage *= 1.5
		score += 10.0 # Bonus for using the skill at optimal range
...
func get_skill_name() -> String: return "Sniper Shot"
func get_skill_description() -> String:
	return "Deal %.1fx base damage to an enemy from long range.\nCooldown: %d turns.\nRange: %d cells." % [damage_mult, max_cooldown, get_skill_range()]
func get_skill_icon() -> Texture: return load("res://assets/ui/skills/sniper_shot.png") # Placeholder
func get_skill_range() -> int: return 6
